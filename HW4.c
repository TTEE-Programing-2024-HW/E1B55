...
//這次的程式作業喔利用跟前一次的圖片印出、輸入密碼跟顯示清單。
//在步驟3的時候，我使用了while迴圈跟for迴圈，因為忘記宣告n的初始值，導致在輸入名字成績時到第四個就自己結束，於是我用printf 顯示n=多少、i=多少才找出錯誤。
//在步驟4的時候，我也一樣利用了while跟for迴圈，去排出他們的名字、學號跟平均成績。
//在步驟5的時候，也是利用while 跟for迴圈，這次有記得宣告初始值。
//在步驟6的時候，也是利用while跟for 迴圈，排成平均成績，之後a、b下去設值在temp裡面。
//在最後一步就是跟上一次的方式一樣去詢問是否要離開。
